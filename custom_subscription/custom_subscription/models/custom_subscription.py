...
class custom_subs_saleOrder(models.Model):
    ...
    customer_ref_no=fields.Char(string="Customer Ref No.", readonly=True)
    customer_po_no= fields.Char(string="Customer PO No." , readonly= True)
...
class sale_to_subs(models.Model):
    ...
    subscribe_dt=fields.Date(string="Subscription Date" , required=True)
...
#         new_price = round((self.price_unit/days_of_month)*total_days_for_bill)
        ...
        if (self.order_id.invoice_count > 1):
            ...
            res = {
                ...
                'origin': self.order_id.name,
                'account_id': account.id,
                'price_unit': new_price, #self.price_unit,
                'quantity': qty,
                'discount': self.discount,
                'uom_id': self.product_uom.id,
                'product_id': self.product_id.id or False,
